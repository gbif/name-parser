...
- **HTML tags are stripped but their text content is kept** (`StripAndStash.stripHtml`), so a tagged connector like `<i>sensu</i> Fabricius, 1780` becomes `sensu Fabricius, 1780` and is picked up as a taxonomic note by the normal note handling — it is *not* removed wholesale.
...
- **Cultivar with a species author before it and a cultivar author after it** (`Acer campestre L. cv. 'Elsrijk' Broerse`) — the mid-string cultivar strip (`StripAndStash.stripQuotedCultivar`) inserts a comma where the cultivar epithet was removed so `L.` and `Broerse` stay two distinct authors instead of gluing into `L.Broerse`.
