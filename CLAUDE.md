...
- **CJK surname-first authors** (`Zhang F`, `Pan Z-X`) are preserved verbatim — surname first, no period after the bare initial. Detected by the absence of a trailing dot on the all-caps part: with a dot it's standard `Surname I.` and we invert to `I.Surname`; without it we keep the original order.
- **Hyphenated initials** (`Chuan-Kwang Yang` → `C.-K. Yang`) preserve the hyphen between the initials and uppercase the second letter: `C.-K.Yang`, not `C.k.Yang`. The convention applies regardless of the input's case (`C.-k. Yang` is normalised to `C.-K.Yang`).
